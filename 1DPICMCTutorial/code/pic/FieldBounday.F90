  ...
  Implicit none
     Type FieldBoundary
                        !Integer(4) :: XStart=0,XEnd=1
                         ...
                         Real(8) :: Dt=Inputdt
                         Real(8) :: Frequency(3)=(/60.d6,60.d6,2.d6/),Voltage(3)=(/0.d0,100.d0,50.d0/)
    ...
    contains
     ...
     Subroutine UpdaterFieldBounday(FB)
            ...
            Select case (FB%FieldBoundaryModel)
                      case (11)
                               FB%V1=FB%Voltage(2)*Dsin(2*PI*FB%Frequency(2)*FB%dt*Dble(FB%Timer))
                               FB%V2=0.d0
										 !FB%V1=FB%Voltage(1)*Dcos(2*PI*FB%Frequency(1)*FB%dt*Dble(FB%Timer))
           !                    FB%V2=0.d0
                               !Write (*,*) FB%V1,FB%V2
                      case (21)
                               FB%V1=FB%Voltage(2)*Dsin(2*PI*FB%Frequency(2)*FB%dt*Dble(FB%Timer))+FB%Voltage(3)*Dsin(2*PI*FB%Frequency(3)*FB%dt*Dble(FB%Timer))
                               FB%V2=0.d0
                      case(22)
                               FB%V1=FB%Voltage(3)*Dsin(2*PI*FB%Frequency(3)*FB%dt*Dble(FB%Timer))
                               FB%V2=FB%Voltage(2)*Dsin(2*PI*FB%Frequency(2)*FB%dt*Dble(FB%Timer))  
                      Case(31)
